  ...
  const validatePlanData = (): ValidationError[] => {
    ...
    if (formData.user_limit) {
      ...
      if (isNaN(userLimit) || userLimit < 0) {
        errors.push({
          field: "user_limit",
          message: "User limit must be a positive number.",
        });
      }
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {plans.map((plan: SubscriptionPlan) => (
              <Card
                key={plan.id}
                className={`relative rounded-2xl border-0 shadow-lg shadow-gray-200/50 hover:shadow-xl transition-all ${
      ...
      {activeTab === "users" && (
        <div className="space-y-4">
          {/* Search and Filter */}
          <Card className="p-5 rounded-2xl border-0 shadow-lg shadow-gray-200/50">
