...
export const adminApi = {
  ...
  getUsers: (page = 1, limit = 20, search?: string) =>
    apiRequest<any>(`/admin/users?page=${page}&limit=${limit}${search ? `&search=${search}` : ''}`),
  ...
  getCourses: (page = 1, limit = 20, topic?: string) =>
    apiRequest<any>(`/courses?page=${page}&limit=${limit}${topic ? `&topic=${topic}` : ''}`),
  
  createCourse: (data: any) => apiRequest<any>('/courses', {
    method: 'POST',
