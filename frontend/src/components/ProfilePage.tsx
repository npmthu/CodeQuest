...
export default function ProfilePage() {
  ...
  const activities = (stats?.recentActivity || []).map((activity: any) => ({
    date: new Date(activity.submitted_at).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
    }),
  ...
  const totalLanguageSubmissions = Object.values(languageStats).reduce(
    (a: number, b: any) => a + (b as number),
    0
  ) as number;
  const skills = Object.entries(languageStats).map(
    ([name, count]: [string, any]) => ({
      name,
      level:
        totalLanguageSubmissions > 0
          ? Math.round((count / totalLanguageSubmissions) * 100)
          : 0,
      problems: count,
    })
  );
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    onClick={() => {
                      const githubUrl =
                        profile?.metadata?.githubUrl || profile?.github_url;
                      if (githubUrl) {
                        window.open(githubUrl, "_blank", "noopener,noreferrer");
                      } else {
                        alert(
                          "GitHub URL not set. Edit your profile to add it."
                        );
                      }
                    }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      const linkedinUrl =
                        profile?.metadata?.linkedinUrl || profile?.linkedin_url;
                      if (linkedinUrl) {
                        window.open(
                          linkedinUrl,
                          "_blank",
                        ...
                        alert(
                          "LinkedIn URL not set. Edit your profile to add it."
                        );
                      }
                    }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      const websiteUrl =
                        profile?.metadata?.websiteUrl || profile?.website_url;
                      if (websiteUrl) {
                        ...
                        alert(
                          "Website URL not set. Edit your profile to add it."
                        );
                      }
                    }}
                    title="Visit website"
